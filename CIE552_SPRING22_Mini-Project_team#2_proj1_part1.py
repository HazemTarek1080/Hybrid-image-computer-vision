...
test_image = cv2.imread('./data/cat.bmp',cv2.IMREAD_COLOR)
test_image = cv2.cvtColor(test_image, cv2.COLOR_BGR2RGB)
plt.imshow(test_image)
print('before function',test_image.shape)
test_image = rescale(test_image, 0.7, mode='reflect',channel_axis=2)
print('before function2',test_image.shape)
'''
Identity filter
...
done = save_image('C:/Users/Dell/PycharmProjects/results/identity_image.jpg', np.clip(identity_image,-1,1))
...
done = save_image(resultsDir + os.sep + 'blur_image.jpg', np.clip(blur_image,-1,1))
...
done = save_image(resultsDir + os.sep + 'sobel_image.jpg', np.clip(sobel_image,-1,1))
...
done = save_image(resultsDir + os.sep + 'laplacian_image.jpg', np.clip(laplacian_image,-1,1))
...
done = save_image(resultsDir + os.sep + 'high_pass_image.jpg', np.clip(high_pass_image,-1,1))
